...
import { fetchMalarticleById } from "../api/apifunction";
...
const QuranStudyContent = ({ activeItem, onPlayAudio }) => {
  ...
  useEffect(() => {
    const loadArticle = async () => {
      ...
      const itemId =
        activeItem?.id || (typeof activeItem === "string" ? 1 : null);
      const itemTitle = activeItem?.title || activeItem || "Unknown Article";
      ...
      try {
        setLoading(true);
        setError(null);
        const data = await fetchMalarticleById(itemId);
        setCurrentArticle(data);
      } catch (err) {
        console.error("Error loading article:", err);
        setError(err.message);

        // Fallback content based on activeItem
        setCurrentArticle({
          id: itemId,
          title: itemTitle,
          content:
            "Content not available offline. Please check your internet connection.",
          matter:
            "Content not available offline. Please check your internet connection.",
        });
      } finally {
  ...
  const getDisplayContent = () => {
    if (!currentArticle) {
      return "Please select an article to view its content.";
    }

    return (
      currentArticle.content ||
      currentArticle.matter ||
      currentArticle.text ||
      currentArticle.body ||
      `Content for ${currentArticle.title} will be displayed here.`
    );
  };
      ...
      <div className="border-b border-gray-300 px-4 lg:px-6 py-4">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
          <h1 className="text-xl lg:text-2xl font-semibold text-black dark:text-white">
            {activeItem?.title || activeItem || "Select an Article"}
          </h1>
          <button
            onClick={onPlayAudio}
            ...
            <>
              {error && (
                <div className="mb-4 p-3 bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200 text-sm rounded">
                  Content loaded from offline cache due to connection issue
                </div>
              )}
              <div className="text-black text-sm lg:text-base leading-relaxed dark:text-white whitespace-pre-wrap">
                {getDisplayContent()}
              </div>
            </>
          )}
