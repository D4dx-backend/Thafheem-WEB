...
import { fetchBanglaFinalityOfProphethood } from '../api/apifunction';

const BanglaFinalityOfProphethood = () => {
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await fetchBanglaFinalityOfProphethood();
        if (!isMounted) return;
      ...
      <div className="max-w-[1070px] w-full mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          <h1 className={`text-3xl font-bold text-gray-900 mb-2 dark:text-white ${isBangla ? 'font-bengali' : ''}`}>
            {isBangla ? 'নবুওয়াতের চূড়ান্ততা' : 'The Finality of Prophethood'}
          </h1>
          <p className="text-gray-600 dark:text-gray-300">
            {isBangla ? 'তাফহীম সম্পদ থেকে বাংলা সামগ্রী।' : 'The Finality of Prophethood - Bangla content from Thafheem resources.'}
          </p>
        ...
        {loading && (
          <div className="py-10 text-center text-gray-600 dark:text-gray-300">
            {isBangla ? 'সামগ্রী লোড হচ্ছে...' : 'Loading content...'}
          </div>
        )}
        ...
        {!loading && !error && sections.length === 0 && (
          <div className="py-10 text-center text-gray-600 dark:text-gray-300">
            {isBangla ? 'সামগ্রী এই মুহূর্তে উপলব্ধ নয়।' : 'Content is not available at the moment.'}
          </div>
        )}
...
export default BanglaFinalityOfProphethood;
