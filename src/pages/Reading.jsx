...
import { Share2, Bookmark, Play, Heart, Info, LibraryBig, Notebook, ChevronLeft, ChevronRight, ArrowUp, X } from "lucide-react";
...
const Reading = () => {
  const { id } = useParams(); // Get Surah ID from URL
  ...
  // Fetch Surah metadata and verses
  useEffect(() => {
    const fetchSurahData = async () => {
      try {
        ...
        const surahResponse = await fetch(`https://thafheem.net/thafheem-api/suranames/${id}`);
        if (!surahResponse.ok) {
        ...
        setSurah({
          id: surahData.SuraID,
          arabicName: surahData.ASuraName,
          englishName: surahData.ESuraName,
          type: surahData.SuraType === "M" ? "Makki" : "Madani",
          totalAyahs: surahData.TotalAyas,
        });

        // Fetch Surah verses (assuming an endpoint like /surah/{id}/verses)
        const versesResponse = await fetch(`https://thafheem.net/thafheem-api/surah/${id}/verses`);
        if (!versesResponse.ok) {
          throw new Error(`Failed to fetch verses: ${versesResponse.status}`);
        }
        const versesData = await versesResponse.json();
        setVerses(versesData); // Expecting array of { id, text }
      } catch (err) {
  ...
  const handlePreviousSurah = () => {
    if (surah && surah.id > 1) {
      navigate(`/surah/${surah.id - 1}`);
    }
  };

  const handleNextSurah = () => {
    if (surah && surah.id < 114) { // Assuming 114 Surahs in the Quran
      navigate(`/surah/${surah.id + 1}`);
    }
  };

  const handleBeginningOfSurah = () => {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-black flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-500 dark:text-red-400 text-lg font-semibold mb-2">Failed to load Surah</p>
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="mb-4 sm:mb-6">
              <h1
                ...
                style={{ fontFamily: "SuraName, Amiri, serif" }}
              >
                {surahNameUnicodes[surah.id]
                  ? String.fromCharCode(parseInt(surahNameUnicodes[surah.id].replace("U+", ""), 16))
                  : surah.arabicName}
              </h1>
              <p className="text-base sm:text-lg text-gray-600 dark:text-gray-400">
                {surah.englishName} • {surah.type} • {surah.totalAyahs} Ayahs
              </p>
            </div>
            ...
            <div className="flex items-center justify-center space-x-3 sm:space-x-4 mb-4 sm:mb-6">
              ...
              <button className="p-2 text-gray-400 dark:text-white hover:text-gray-600 dark:hover:text-gray-300 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center">
                <Heart className="w-4 h-4 sm:w-5 sm:h-5 text-black dark:text-white" />
              </button>
            </div>

            {/* Bismillah */}
            {surah.id !== 9 && ( // Surah At-Tawbah (9) does not have Bismillah
              <div className="mb-6 sm:mb-8">
                <img
                  src={Bismi}
                  alt="Bismillah"
                ...
                <div key={verse.id}>
                  <p
                    ...
                    className={`text-base sm:text-lg md:text-xl lg:text-2xl xl:text-2xl font-arabic leading-loose text-gray-900 dark:text-white mb-3 sm:mb-4 px-2 ${
                      index === 0 ? "bg-[#E7F0F3] dark:bg-[#63C3DC] py-1 rounded" : ""
                    }`}
                  >
                    {verse.text} ﴿{verse.id}﴾
                  </p>
                  {(index + 1) % 5 === 0 && index !== verses.length - 1 && (
                    <>
        ...
        <div className="bg-white border-t dark:bg-black border-gray-200 dark:border-gray-700 px-3 sm:px-4 py-3 sm:py-4 mt-6 sm:mt-8">
          ...
          <div className="sm:hidden space-y-2">
            <div className="flex space-x-2">
              <button
                ...
                disabled={surah.id === 1}
                className="flex-1 flex items-center justify-center space-x-2 px-4 py-2 text-xs text-gray-600 dark:bg-[#323A3F] dark:text-white hover:text-gray-900 dark:hover:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg w-[173.96px] min-h-[44px] disabled:opacity-50"
              >
                <ChevronLeft className="w-3 h-3" />
                <span>Previous Surah</span>
              </button>
              <button
                onClick={handleBeginningOfSurah}
                className="flex-1 flex items-center justify-center space-x-2 px-4 py-2 text-xs text-gray-600 dark:bg-[#323A3F] dark:text-white hover:text-gray-900 dark:hover:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg min-h-[44px]"
              >
                <ArrowUp className="w-3 h-3" />
                <span>Beginning of Surah</span>
              </button>
            ...
            <div className="flex justify-center">
              <button
                ...
                className="w-[173.96px] flex items-center justify-center space-x-2 px-4 py-2 text-xs text-gray-600 dark:bg-[#323A3F] dark:text-white hover:text-gray-900 dark:hover:text-gray-300 border border-gray-300 dark:border-gray-600 rounded-lg min-h-[44px] disabled:opacity-50"
              >
                <span>Next Surah</span>
                <ChevronRight className="w-3 h-3" />
              </button>
          ...
          <div className="hidden sm:flex max-w-4xl mx-auto items-center justify-center space-x-4 lg:space-x-6">
            <button
              onClick={handlePreviousSurah}
              disabled={surah.id === 1}
              className="flex items-center space-x-2 px-3 sm:px-4 py-2 text-xs sm:text-sm text-gray-600 dark:bg-[#323A3F] dark:text-white hover:text-gray-900 dark:hover:text-gray-300 rounded-lg disabled:opacity-50"
            >
              <ChevronLeft className="w-3 h-3 sm:w-4 sm:h-4" />
              <span>Previous Surah</span>
            </button>
            <button
              onClick={handleBeginningOfSurah}
              className="flex items-center space-x-2 px-3 sm:px-4 py-2 text-xs sm:text-sm text-gray-600 dark:bg-[#323A3F] dark:text-white hover:text-gray-900 dark:hover:text-gray-300 rounded-lg"
            >
              <ArrowUp className="w-3 h-3 sm:w-4 sm:h-4" />
              <span>Beginning of Surah</span>
            </button>
